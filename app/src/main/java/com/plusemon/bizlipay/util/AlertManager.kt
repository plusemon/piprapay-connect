...
object AlertManager {
    ...
    /**
     * Triggers a short single haptic vibration pulse for destructive action confirmations.
     */
    fun triggerDestructiveHaptic(context: Context) {
        try {
            ...
            Log.w(TAG, "Destructive haptic failed: ${e.message}")
        }
    }
    ...
    private fun getOrCreatePosChimeWav(context: Context): File {
        val file = File(context.cacheDir, "bizlipay_pos_chime.wav")
